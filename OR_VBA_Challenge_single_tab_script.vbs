' Script for analyzing a single tab

Sub StockMarketAnalysisSingleTab():
...
' Set the loop to calculate the Greatest values
            LastRowOfSummary = Cells(Rows.Count, 1).End(xlUp).Row

            For i = 2 To LastRowOfSummary
                If Cells(i, 11) = Greatest_Increase Then
                    Greatest_Increase_Ticker_Name = Cells(i, 11).Offset(0, -2).Value
                End If
                    
                If Cells(i, 11) = Greatest_Decrease Then
                    Greatest_Decrease_Ticker_Name = Cells(i, 11).Offset(0, -2).Value
                End If
                
                If Cells(i, 12) = Greatest_Total_Volume Then
                    Greatest_Total_Volume_Ticker_Name = Cells(i, 12).Offset(0, -3).Value
                End If
            Next i
